...
pub(crate) const MERGE_EXCERPT_WORDS: usize = 500;
...
pub(crate) fn head_words(text: &str, n: usize) -> String {
    let words: Vec<&str> = text.split_whitespace().collect();
...
pub async fn run_continuous_mode<C: crate::run_context::RunContext>(
    ...
    // Delegate shutdown + flush to the flush_on_stop component.
    // See docs/superpowers/plans/2026-04-18-continuous-mode-test-harness.md
    // for the extraction rationale.
    let flush_deps = crate::continuous_mode_flush_on_stop::FlushOnStopDeps {
        handle: handle.clone(),
    ...
    let flush_handles = crate::continuous_mode_flush_on_stop::FlushOnStopHandles {
        pipeline_handle,
        sensor_handle,
        consumer_task,
        detector_task,
    ...
    crate::continuous_mode_flush_on_stop::run(&ctx, flush_deps, flush_handles).await?;
