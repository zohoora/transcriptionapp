...
//!
//! Stable check names: `clinical_classification`, `billing_codes`,
//! `billing_codes_unexpected:{CODE}`, `billing_quantity:{CODE}`,
//! `diagnostic_code`, `billing_json_missing`, `date_parse`.
//!
//! ## Usage
//!
...
///
/// Each check produces a name (stable identifier) and pass/fail. Failures are
/// partitioned into "expected" (in `label.expected_failures`) and "regressions"
/// (everything else). "Drift" tracks names that WERE listed as expected but
/// passed in this run — surfaces tightening opportunities.
#[derive(Debug, Default)]
struct CheckResults {
    checks: u32,
    passes: u32,
    /// Unexpected failures. Trigger `--fail-on-regression`.
    regressions: Vec<String>,
    /// Expected failures that fired as expected. Counted, not blocking.
    expected_fired: Vec<String>,
    /// Names listed in `expected_failures` that did NOT fire — corpus drift,
    /// tightening opportunity. Informational only.
    drift: Vec<String>,
    /// All check names that failed in this run, regardless of expectation.
    /// Used by `--bootstrap-expected-failures` to write the new baseline.
    all_failed_names: BTreeSet<String>,
}
...
impl CheckResults {
    ...
    fn record_fail(&mut self, name: &str, msg: String, expected_failures: &[String]) {
        self.checks += 1;
        self.all_failed_names.insert(name.to_string());
        if expected_failures.iter().any(|n| n == name) {
            self.expected_fired.push(msg);
        } else {
            self.regressions.push(msg);
        }
    }
...
fn rewrite_expected_failures(
    ...
    let next = if new_array.is_empty() {
        ...
        // a noisy `[]`. Skip rewrite if it was already absent.
...
async fn main() -> ExitCode {
    ...
    let mut total_expected_fired = 0;
    let mut total_drift = 0;
    let mut missing_sessions = 0;
    let mut bootstrapped_files = 0;
    ...
    for file in &label_files {
        ...
        let label_name = file
            ...
            .and_then(|n| n.to_str())
            .unwrap_or("?");
        let expected_failures: Vec<String> = label
            .labels
            .expected_failures
            .clone()
            .unwrap_or_default();
        ...
        if want_billing {
            let parsed_date = match parse_date(&label.date) {
                ...
                None => {
                    ...
                    results.record_fail("date_parse", msg, &expected_failures);
                    finalize_label(
                        label_name,
                        &label,
                        &results,
                        ...
                        &mut total_checks,
                        &mut total_passes,
                        &mut total_regressions,
                        &mut total_expected_fired,
                        &mut total_drift,
                        &mut bootstrapped_files,
                    );
                    continue;
                }
            };
                ...
                Some(billing) => {
                    ...
                    if let Some(expected_codes) = &label.labels.billing_codes_expected {
                        if expected_codes.iter().all(|c| actual_codes.contains(c)) {
                            results.record_pass("billing_codes", &expected_failures);
                        } else {
                            ...
                            results.record_fail("billing_codes", msg, &expected_failures);
                        }
                    }

                    if let Some(unexpected_codes) = &label.labels.billing_codes_unexpected {
                        for code in unexpected_codes {
                            let name = format!("billing_codes_unexpected:{}", code);
                    ...
                    if let Some(qty_expected) = &label.labels.billing_quantity_expected {
                        for (code, expected_qty) in qty_expected {
                            let name = format!("billing_quantity:{}", code);
                    ...
                    if let Some(expected_dx) = &label.labels.diagnostic_code_expected {
                        ...
                        if &actual_dx == expected_dx || acceptable.contains(&actual_dx) {
                            results.record_pass("diagnostic_code", &expected_failures);
                        } else {
                            let msg = format!(
                                "  ✗ diagnostic_code: expected {} (acceptable: {:?}), got {}",
                                expected_dx, acceptable, actual_dx
                            );
                            results.record_fail("diagnostic_code", msg, &expected_failures);
                        }
                    }
                }
                None => {
                    let msg =
                        "  ✗ billing_json_missing: billing.json not found, cannot check codes/dx"
                            .to_string();
                    results.record_fail("billing_json_missing", msg, &expected_failures);
                }
            }
        }
    ...
    println!(
        ...
        total_checks,
        total_passes,
        total_expected_fired,
        total_regressions,
        total_drift,
        missing_sessions
    );
    if bootstrap {
        println!("Bootstrapped {} label files.", bootstrapped_files);
    }
    ...
    if fail_on_regression && total_regressions > 0 {
        ...
        eprintln!(
            ...
            total_regressions, total_labels
        );
...
fn finalize_label(
    ...
    total_passes: &mut u32,
    total_regressions: &mut u32,
    total_expected_fired: &mut u32,
    total_drift: &mut u32,
    bootstrapped_files: &mut u32,
) {
    *total_checks += results.checks;
    *total_passes += results.passes;
    *total_regressions += results.regressions.len() as u32;
    *total_expected_fired += results.expected_fired.len() as u32;
    *total_drift += results.drift.len() as u32;

    let regressed = !results.regressions.is_empty();
    let interesting = regressed
        || !results.drift.is_empty()
        || !results.expected_fired.is_empty()
        || verbose;

    if interesting {
        let label_status = if regressed {
            "REGRESSION"
        } else if !results.expected_fired.is_empty() {
            "expected"
        } else if !results.drift.is_empty() {
            "drift"
        } else {
            "OK"
        };
        println!(
            "{} {} ({} checks, {} pass)",
            label_status, label_name, results.checks, results.passes
        );
        if let Some(notes) = &label.labels.notes {
            println!("  notes: {notes}");
        }
        for fail in &results.regressions {
            println!("{}", fail);
        }
        for fail in &results.expected_fired {
            // Prefix the message with `≈` to distinguish from regressions
            // when reading the log.
            println!("  ≈ (expected) {}", fail.trim_start());
        }
    ...
    if bootstrap {
        match rewrite_expected_failures(file, &results.all_failed_names) {
            Ok(true) => *bootstrapped_files += 1,
            Ok(false) => { /* unchanged */ }
...
mod tests {
    ...
    fn unexpected_failure_is_a_regression() {
        ...
        r.check_eq("billing_codes", &"A".to_string(), &"B".to_string(), &no_expected());
        assert_eq!(r.regressions.len(), 1);
        assert!(r.expected_fired.is_empty());
        assert!(r.drift.is_empty());
        assert!(r.all_failed_names.contains("billing_codes"));
    }

    #[test]
    fn expected_failure_is_not_a_regression() {
        let mut r = CheckResults::default();
        let expected = vec!["billing_codes".to_string()];
        r.check_eq("billing_codes", &"A".to_string(), &"B".to_string(), &expected);
    ...
    fn expected_pass_surfaces_drift() {
        let mut r = CheckResults::default();
        let expected = vec!["billing_codes".to_string()];
        r.check_eq("billing_codes", &"A".to_string(), &"A".to_string(), &expected);
        assert_eq!(r.passes, 1);
        assert_eq!(r.drift, vec!["billing_codes".to_string()]);
        assert!(r.regressions.is_empty());
        assert!(r.expected_fired.is_empty());
    }
    ...
    fn unexpected_pass_does_not_surface_drift() {
        let mut r = CheckResults::default();
        r.check_eq("billing_codes", &"A".to_string(), &"A".to_string(), &no_expected());
        assert_eq!(r.passes, 1);
        assert!(r.drift.is_empty());
    }
    ...
    fn record_fail_with_expected_name_is_expected() {
        let mut r = CheckResults::default();
        let expected = vec!["billing_codes_unexpected:K005A".to_string()];
        r.record_fail(
            "billing_codes_unexpected:K005A",
            "  ✗ billing_codes_unexpected:K005A: code K005A present".to_string(),
            &expected,
        );
        assert_eq!(r.expected_fired.len(), 1);
        assert!(r.regressions.is_empty());
    ...
    fn record_fail_unexpected_name_is_a_regression() {
        let mut r = CheckResults::default();
        let expected = vec!["billing_codes_unexpected:OTHER".to_string()];
        r.record_fail(
            "billing_codes_unexpected:K005A",
            "  ✗ billing_codes_unexpected:K005A: code K005A present".to_string(),
            &expected,
...
"#,
        ...
        failures.insert("billing_codes".to_string());
        failures.insert("diagnostic_code".to_string());
        ...
        assert_eq!(names, vec!["billing_codes", "diagnostic_code"]);
    }
