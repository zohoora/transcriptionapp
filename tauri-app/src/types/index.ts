...
export interface LocalArchiveSummary {
  ...
  /** Splitter's audio-vs-vision cross-check flagged this session — clinician should verify the patient name. (v0.10.67+) */
  chart_stale_suspected?: boolean | null;
}
...
export interface LocalArchiveMetadata {
  ...
  room_name?: string | null;
  /**
   * True when the audio-vs-vision cross-check at split time disagreed with the
   * vision-derived patient_name (e.g. EMR chart was on the wrong patient).
   * History view renders a "verify patient" badge on these rows. Absent on
   * sessions where the check passed or wasn't run. (v0.10.67+)
   */
  chart_stale_suspected?: boolean | null;
}
...
export type ContinuousModeEventType =
  ...
  | 'chart_stale_suspected';

/** Why we suspect the EMR chart is on the wrong patient. */
export type ChartStaleReason = 'multi_chart' | 'audio_mismatch';
...
export interface ContinuousModeEvent {
  ...
  dob?: string;
  /** For chart_stale_suspected: which signal fired (multi_chart vs audio_mismatch) */
  reason?: ChartStaleReason;
  /** For chart_stale_suspected: vision-derived majority name at detection */
  vision_name?: string;
  /** For chart_stale_suspected: distinct names vision has seen this encounter */
  vision_unique_names?: string[];
  /** For chart_stale_suspected (audio_mismatch): greeting names parsed from transcript */
  audio_greeting_candidates?: string[];
}
