...
| Confirm-patient dual-write (v0.10.46+) | Clinician presses "Confirm Patient" in History Window → `confirm_session_patient` Tauri command writes to local archive, Medplum FHIR (if authenticated), and profile-service patient index IN ORDER, so UI can show per-store status inline. Both remote stores carry the Medplum FHIR ID as the canonical `patient_id` (profile-service falls back to UUID when Medplum unreachable, reconciled on next sync). Idempotent on (name_normalized, dob) — repeated confirms append session_id (deduped). DOB is REQUIRED to fire the dual-write; rename-only flows use the existing `update_session_patient_name`. Medplum path: `upsert_patient_by_name_dob` → `create_encounter` → upload SOAP + transcript DocumentReferences → `complete_encounter_with_period`. Profile-service path: `POST /physicians/:id/patients/confirm` → `PatientRecord` with session back-link. Name normalization duplicated in both stores with a parity test to prevent idempotency drift. See ADR-0030 |
