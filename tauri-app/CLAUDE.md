...
| Diagnostic code semantic guard (v0.10.67+) | `dx_description_matches_primary` (rule_engine.rs) gates Stage 0 tools-model output by lexical overlap (≥4-char tokens with stopwords removed) + a known-synonym table (DX_SYNONYMS — acne↔sebaceous, knee↔osteoarthritis, hypertension↔hypertensive, diarrhea↔gastro-enteritis, etc.) + an age-restriction veto (descriptions containing "childhood"/"infant"/"neonatal" reject when the primary diagnosis lacks a pediatric anchor). When tools-model returns a code whose DB description is unrelated to primary_diagnosis, Stage 0 falls through to Stage 2 text match. Single parameterized table-driven test pins all 2026-04-29 failure modes (Carter acne→706 not 701, Irene cervicogenic→729 not 491, Daniel adult-ADHD veto, Catherine knee→715 not 249, Janice HTN→401 not 311) |
