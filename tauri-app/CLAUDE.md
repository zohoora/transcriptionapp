...
| Vision early-stop | Screenshot task skips the vision LLM call (but NOT the screenshot capture/archive) once `PatientNameTracker::should_skip_vision(K, cap)` returns true. Two OR'd conditions: `streak_count >= K` (K=5 consecutive matching recorded names) OR `vision_calls_attempted >= cap` (cap=30 pathological backstop). Tracker maintains `streak_name`/`streak_count` in `record()` and a `vision_calls_attempted` counter bumped by `note_vision_attempt()` before each call (including failures). All three state fields reset in `PatientNameTracker::reset()` on encounter split. Calibrated from Apr 16 2026 audit: ~78% vision call reduction with no downstream behavior change on stable encounters |
