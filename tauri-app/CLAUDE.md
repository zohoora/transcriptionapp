...
| LLM/SOAP (6) | `check_ollama_status`, `list_ollama_models`, `prewarm_ollama_model`, `generate_soap_note` (supports `model_override` + `session_date` for cross-day per-patient regen), `generate_soap_note_auto_detect` (supports `model_override`), `generate_predictive_hint` | `commands/ollama.rs` |
...
| `continuous_mode_event` | Continuous mode status changes (started, encounter_detected, soap_generated, encounter_merged, sensor_status, shadow_decision, retrospective_split, sleep_started, sleep_ended, chart_stale_suspected, etc.) |
...
| Chart-stale cross-check (v0.10.67+) | Defense in depth for the EMR-chart-on-wrong-patient failure mode (2026-04-29 Catherine 2:35 = Shirley Rice, Sara 2:50 = Jaden Slote). Three signals: (1) `PatientNameTracker::is_chart_likely_stale()` returns true when ≥3 distinct names have been recorded — `screenshot_task` emits `ChartStaleSuspected{reason: MultiChart}` once per encounter, reset on `last_split_time` bump; (2) `extract_greeting_name_candidates(transcript, prefix_words)` parses common openers ("Hi/Hello [Name]", "Mr/Mrs/Ms [Name]") and skips doctor-self-introductions ("This is Dr X"); (3) `cross_check_vision_vs_audio(vision_name, audio_candidates) -> NameCrossCheck::{Match,Mismatch,Inconclusive}` with English nickname bridge (Cathy↔Catherine etc.) — `continuous_mode_splitter` runs the cross-check after split, emits `ChartStaleSuspected{reason: AudioMismatch}` on mismatch, and stamps `chart_stale_suspected: true` on metadata via `local_archive::mark_chart_stale_suspected`. `ChartStaleReason` enum is `MultiChart` / `AudioMismatch` (snake_case-serialized for the frontend). Frontend renders a "verify patient" banner; the vision majority is still applied as the patient name unless the clinician overrides |
...
| Procedure-section discipline (v0.10.67+) | SOAP prompt has explicit POSITIVE list (injections, biopsy, excision, suture, cryotherapy, pap, IUD, etc.) AND NEGATIVE list (chest auscultation, palpation, lab review, providing copies, completing forms, prescription renewal, telephone call). Rendered SOAP shows action only — `transcript_quote` stays in JSON for billing audit but does NOT leak into the user-visible SOAP body. After parse, `filter_billable_procedures()` post-filters via `billing::procedure_vocab::is_billable_procedure_action(&str)` which case-insensitive-substring-matches a curated `BILLABLE_PROCEDURE_VERBS` list. Same vocabulary is used by the regression CLI's `procedure_section_judges_correct` heuristic so SOAP renderer + regression scoring share one source of truth. Caught the 2026-04-29 overcapture failure modes (chest auscultation listed as procedure, "reviewed blood work", DTC form completion, etc.) |
| Diagnostic code semantic guard (v0.10.67+, expanded v0.10.68+) | `dx_description_matches_primary` (rule_engine.rs) gates Stage 0 tools-model output by lexical overlap (≥4-char tokens with stopwords removed) + a known-synonym table (DX_SYNONYMS — acne↔sebaceous, knee↔osteoarthritis, hypertension↔hypertensive, diarrhea↔gastro-enteritis, **radiculopathy↔lumbago/sciatica, peroneal↔neuritis, nerve entrapment↔neuritis** added 2026-04-30) + an age-restriction veto. **2026-04-30**: Stage 1 (LLM `suggestedDiagnosticCode`) ALSO runs through `dx_obvious_category_mismatch` at high confidence (catches musculoskeletal-vs-respiratory orthogonality without rejecting borderline cases). `match_diagnosis_text` stop-word list expanded with `no/none/diagnosis/documented/primary` (catches Karin Smit-style "No diagnosis documented" → 311 stop-word degeneracy). Buggy `prp → skin subcutaneous` abbreviation expansion REMOVED (Joanne Takacs case landed on 707 bedsore for chronic-pain post-PRP). Pinned by parameterized table-driven tests across 2026-04-29 (Carter acne→706, Irene cervicogenic→729, Daniel adult-ADHD veto, Catherine knee→715, Janice HTN→401) AND 2026-04-30 (James/Linda radiculopathy→724, Carl peroneal→356, Karin no-diagnosis→None, Joanne PRP/RA→not-707) failure modes. |
| K005A duration scaling (v0.10.68+) | K005A/K007A added via the `Conditions::PrimaryMentalHealth` / `OpioidWithdrawalManagement` branch now scales `quantity` from session duration via `counselling_units_from_duration` — previously only the assessment_code path did this, leaving Conditions-branch K005 stuck at quantity=1. `k005_already_added`/`k007_already_added` flags prevent dual-condition over-billing (Dorothy Apr 27 case). Multi-condition same-K-code now produces correct duration-based qty (was N×qty=1 dedup-summed) |
| K-code SOAP keyword guard (v0.10.68+) | `condition_keyword_guard` extended with `PrimaryMentalHealth` (anxiety/depression/counselling/psychotherapy/anger management/PTSD/trauma/mood/bipolar/OCD/suicid/self-harm) and `FibromyalgiaCare` (fibromyalgia/myalgic encephalomyelitis/ME/CFS/chronic fatigue syndrome) keyword arms. K005 dropped when SOAP has no MH content (Karen White cervical+MS case); K037 dropped when SOAP has only RA, not fibromyalgia (Joanne Takacs case) |
...
| Procedure-section anatomy + compound rules (v0.10.68+) | `augment_procedures_from_soap_text` keyword table extended with `epidural`, `cervical injection`, `neck injection`, `back injection`, `lumbar injection`, `spinal injection` (single-substring) AND a compound (AND-pattern) signal table for cases where the SOAP wording places verb + anatomy several words apart (e.g., "Administered injection to right side of neck"). The Karen White compound case requires `injection AND neck` — neither single-substring `neck injection` nor `to neck` matches her literal SOAP text. Compound rules: 13 verb-anatomy pairs covering nerve blocks, joint injections, paravertebral. Returns LIST of soap-grounded ProcedureTypes (both newly-added AND LLM-extracted-then-corroborated) so the rule engine can bypass past-tense transcript validation for SOAP-grounded procedures |
| validate_procedure_evidence SOAP-grounded bypass (v0.10.68+) | `RuleEngineContext.soap_procedure_text: Option<String>` — when set, `map_features_to_billing_with_tools_model` runs `augment_procedures_from_soap_text_tracked` internally and bypasses `validate_procedure_evidence` (past-tense transcript check) for SOAP-grounded procedures. Fixes the silent-doctor-narration class: Karen White ("Administered injection to right side of neck") and Frederick Moore ("Administered knee injection") had real injections documented in SOAP but no past-tense action language in transcript — pre-fix the procedure was silently dropped. Replay/experiment tools now also benefit since the rule engine itself owns the augment + bypass logic |
...
| SOAP P/Procedure contradiction detector (v0.10.68+) | `encounter_pipeline::detect_soap_p_procedure_contradiction(soap)` — fires when Procedure section is non-empty AND P-section contains a deferral phrase (`hold off on (further/the) injection`, `no injection performed`, `did not inject`, `injection deferred`, etc.). Wired into `extract_and_archive_billing`: emits `soap_contradiction_detected` event for clinician review but does NOT block billing (Procedure section is canonical record of what physically happened). James Dollery's case: P "Hold off on further injections at this time" + Procedure "Performed injection at left-sided painful spot" — captured the doctor verbalizing the decision flip mid-sentence ("Did you want to do an injection? Yeah. Do you wanna hold off") |
...
| Drug-name verbatim rule (v0.10.67+) | SOAP prompt's `Rules:` section now instructs: when a drug name in the transcript is unfamiliar or phonetically uncertain, keep it verbatim and append "(transcribed; verify spelling)" — do NOT substitute a similar-sounding drug. Closes the 2026-04-29 Janice failure mode where "Davigo" (likely Dayvigo / lemborexant) was annotated as estradiol |
| Cross-machine sync allowlist (v0.10.67+) | `ServerSyncContext::SYNCED_AUX_FILES` constant in `server_sync.rs` is the single source of truth for the file list uploaded to the profile service. Must match `ALLOWED_SESSION_FILES` in `profile-service/src/store/sessions.rs`; a parity test (`test_synced_aux_files_matches_profile_service_allowlist`) enforces this. Adding a synced file requires updating both lists + the test fixture. Includes `patient_labels.json` (v0.10.67) so cross-room per-patient SOAP regen can read the per-patient summary |
...
# SOAP / billing prompt-engineering experiments (v0.10.62+, --replay-only added v0.10.67)
cargo run --bin soap_experiment_cli -- --date 2026-04-29 --variant v0_10_61 --variant no_procsec --seeds 3
cargo run --bin billing_experiment_cli -- --date 2026-04-29 --variant baseline --variant visit_dx --seeds 3
# --replay-only mode: re-score archived response_raw from replay_bundle.json without live LLM (offline, requires schema v5+ bundles)
cargo run --bin soap_experiment_cli -- --date 2026-04-29 --variant v0_10_61 --replay-only
cargo run --bin billing_experiment_cli -- --date 2026-04-29 --variant baseline --replay-only

# Labeled regression: compares production output to ground-truth labels in tests/fixtures/labels/
cargo run --bin labeled_regression_cli -- --all
cargo run --bin labeled_regression_cli -- --all --fail-on-regression  # CI gate
# Checks billing_codes_expected, diagnostic_code_expected, clinical_correct, and (v0.10.67+) procedure_section_correct
# via replay_bundle.json::soap_result.response_raw + billing::procedure_vocab::is_billable_procedure_action
```
