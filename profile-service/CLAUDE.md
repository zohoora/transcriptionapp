...
| File allowlist | `is_allowed_session_file()` in `store/sessions.rs` — explicit allowlist for auxiliary files. Currently allows: `pipeline_log.jsonl`, `replay_bundle.json`, `segments.jsonl`, `billing.json`, `patient_handout.txt`, `clinician_notes.json`, `patient_labels.json` (added v0.10.67 — needed for cross-room per-patient SOAP regen), and `screenshots/*.jpg`. **Note**: `feedback.json` is not in the allowlist — uses a dedicated typed route. Adding new aux file types requires updating BOTH the allowlist AND the tauri-side `server_sync.rs::SYNCED_AUX_FILES`; a parity test in `server_sync.rs::test_synced_aux_files_matches_profile_service_allowlist` enforces the alignment |
...
| Modify file allowlist | `store/sessions.rs` (`is_allowed_session_file()`) — currently allows: pipeline_log, replay_bundle, segments, screenshots/*.jpg, billing.json, patient_handout.txt, clinician_notes.json, patient_labels.json. Update tauri-side `SYNCED_AUX_FILES` in `server_sync.rs` to match (parity test enforces) |
